...
This is direct reuse of Control-Flow Narrowing.

## 9. Early return, loops, and switch

The statement layer tracks exit behavior and uses the existing narrowing state. Expression and Program Checking adds body checking for `while`, `for`, and `switch`, while early-return fixtures establish that narrowing on a surviving path must remain available.
...
Expression and Program Checking tests deliberately preserve useful diagnostics after earlier errors. The general policy is:
...
By Expression and Program Checking the flow is:
...
## 14. What Expression and Program Checking completed
